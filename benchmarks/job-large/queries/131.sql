SELECT t_1.title AS american_vhs_movie_1,
       t_2.title AS american_vhs_movie_2
...
WHERE ct_1.kind = 'production companies'
  AND mc_1.note LIKE '%(VHS)%'
  AND mc_1.note LIKE '%(USA)%'
  AND mc_1.note LIKE '%(1994)%'
  AND mi_1.info IN ('USA',
                    'America')
  ...
  AND mc_2.note LIKE '%(VHS)%'
  AND mc_2.note LIKE '%(USA)%'
  AND mc_2.note LIKE '%(1994)%'
  AND mi_2.info IN ('USA',
                    'America')
