SELECT mi_1.info AS budget_1,
       t_1.title AS unsuccsessful_movie_1,
       mi_2.info AS budget_2,
       t_2.title AS unsuccsessful_movie_2
...
WHERE cn_1.country_code ='[us]'
  ...
  AND (ct_1.kind ='production companies'
       OR ct_1.kind = 'distributors')
  AND it1_1.info ='budget'
  AND it2_1.info ='bottom 10 rank'
  AND t_1.production_year >2000
  AND (t_1.title LIKE 'Birdemic%'
       OR t_1.title LIKE '%Movie%')
  ...
  AND (ct_2.kind ='production companies'
       OR ct_2.kind = 'distributors')
  AND it1_2.info ='budget'
  AND it2_2.info ='bottom 10 rank'
  AND t_2.production_year >2000
  AND (t_2.title LIKE 'Birdemic%'
       OR t_2.title LIKE '%Movie%')
