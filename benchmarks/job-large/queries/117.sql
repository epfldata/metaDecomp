SELECT n_1.name, t_1.title, chn_2.name, n_2.name, t_2.title
FROM movie_companies AS mc_1,
    ...
    role_type AS rt_2
WHERE ci_1.note IN ( '(voice)' , '(voice: Japanese version)' , '(voice) (uncredited)' , '(voice: English version)' )
  AND n_1.name LIKE '%Ang%'
  AND n_1.gender = 'f'
  AND cn_1.country_code = '[us]'
  AND rt_1.role = 'actress'
  AND t_1.production_year BETWEEN 2005 AND 2009
...
GROUP BY n_1.name, t_1.title, chn_2.name, n_2.name, t_2.title;