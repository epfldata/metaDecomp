SELECT n_1.name AS voicing_actress_1,
       t_1.title AS kung_fu_panda_1,
       n_2.name AS voicing_actress_2,
       t_2.title AS kung_fu_panda_2
FROM aka_name AS an_1,
     ...
     title AS t_1,
     title AS t_2
WHERE ci_1.note = '(voice)'
  AND cn_1.country_code ='[us]'
  AND it_1.info = 'release dates'
  AND mc_1.note LIKE '%(200%)%'
  AND (mc_1.note LIKE '%(USA)%'
       OR mc_1.note LIKE '%(worldwide)%')
  AND mi_1.info IS NOT NULL
  AND (mi_1.info LIKE 'Japan:%2007%'
       OR mi_1.info LIKE 'USA:%2008%')
  AND n_1.gender ='f'
  AND n_1.name LIKE '%Angel%'
  AND rt_1.role ='actress'
  AND t_1.production_year BETWEEN 2007 AND 2008
  AND t_1.title LIKE '%Kung%Fu%Panda%'
  ...
  AND (mc_2.note LIKE '%(USA)%'
       OR mc_2.note LIKE '%(worldwide)%')
  AND mi_2.info IS NOT NULL
  AND (mi_2.info LIKE 'Japan:%2007%'
       OR mi_2.info LIKE 'USA:%2008%')
  AND n_2.gender ='f'
  AND n_2.name LIKE '%Angel%'
  AND rt_2.role ='actress'
  AND t_2.production_year BETWEEN 2007 AND 2008
  AND t_2.title LIKE '%Kung%Fu%Panda%'
