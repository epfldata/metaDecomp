SELECT n_1.name, t_1.title, mi_2.info, mi_idx_2.info, n_2.name, t_2.title
FROM name AS n_1,
    aka_name AS an_1,
    title AS t_1,
    movie_link AS ml_1,
...
WHERE an_1.name LIKE '%a%'
  ...
  AND ( n_1.gender = 'm' OR ( n_1.gender = 'f' AND n_1.name LIKE 'B%' ) )
  AND it_1.info = 'mini biography'
  AND lt_1.link = 'features'
  AND t_1.production_year BETWEEN 1980 AND 1995
  AND ci_1.movie_id = t_1.id
  AND ci_1.movie_id = ml_1.linked_movie_id
  AND t_1.id = ml_1.linked_movie_id
...
GROUP BY n_1.name, t_1.title, mi_2.info, mi_idx_2.info, n_2.name, t_2.title;