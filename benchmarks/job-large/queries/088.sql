SELECT cn_1.name AS producing_company_1,
       ...
       t_1.title AS movie_about_winning_1,
       cn_2.name AS producing_company_2,
       miidx_2.info AS rating_2,
       t_2.title AS movie_about_winning_2
...
WHERE cn_1.country_code ='[us]'
  ...
  AND kt_1.kind ='movie'
  AND t_1.title != ''
  AND (t_1.title LIKE '%Champion%'
       OR t_1.title LIKE '%Loser%')
  ...
  AND kt_2.kind ='movie'
  AND t_2.title != ''
  AND (t_2.title LIKE '%Champion%'
       OR t_2.title LIKE '%Loser%')
