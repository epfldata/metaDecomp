...
import decompositions.{Hypergraph, MetaDecompBasedOptimizer, MetaDecompCyclicOptimizer, MetaDecompGraph, MetaDecompGraphConstructBaseline, MetaDecompGraphConstructInterpolatable, MetaDecompGraphConstructor, MetaDecompGraphRandomizedConstructor, PlanNode, metaGYO}
...
object MetaDecompRandomRunner extends BaseRunner {
  def main(args: Array[String]): Unit = {
    for (benchmark <- if args.size >= 2 then List(args(1)) else benchmarks) {
      connect(benchmark)

      val benchmarkPath = s"$benchmarksPath/$benchmark"
      val resultsPath = Paths.get(resultsDir, s"metadecomp-opt-randomized-$benchmark-$getTimestamp.csv")
      Files.write(
        resultsPath,
      ...
      for (_ <- 0 until 1) {
        ...
        sqlFilesInBenchmark(benchmark).filter(f => if args.size >= 3 then f.getName.matches(args(2)) else true).foreach(sqlFile =>
          ...
          if (Files.exists(Paths.get(benchmarkPath, "cardinalities", s"${queryName}.csv"))) {
            ...
            if (metaGYO(sqlIR.hyperedges).isEmpty) { // Cyclic
              ...
              while ( {
                meta = MetaDecompGraphRandomizedConstructor().run(hypergraph, width)
              ...
              var planList: ArrayBuffer[PlanNode] = ArrayBuffer.empty[PlanNode]
              val planningTime = (for (i <- 0 until repeatTimes) yield {
                meta = MetaDecompGraphRandomizedConstructor().run(hypergraph, width)
              ...
              var i = 0
              var inCostList: ArrayBuffer[Double] = ArrayBuffer.empty[Double]
              var intCostList: ArrayBuffer[Double] = ArrayBuffer.empty[Double]
              val executionTime = (for (plan <- planList) yield {
                ...
                val execTime = runPlan(plan)
                println(s"Execution run $i: $execTime us")
                i += 1
                val intermediateCost = plan.intermediateCost
                intCostList.append(intermediateCost)
                val inCost = plan.inputCost
                inCostList.append(inCost)
                execTime
              }).sorted.apply(repeatTimes / 2)
              ...
              val intermediateCost = intCostList.sorted.apply(repeatTimes / 2)
              val inCost = inCostList.sorted.apply(repeatTimes / 2)
