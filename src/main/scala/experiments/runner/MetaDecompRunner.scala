...
object MetaDecompRunner extends BaseRunner {
	def main(args: Array[String]): Unit = {
		for (benchmark <- if args.size >= 2 then List(args(1)) else benchmarks) {
			...
			val resultsPath = Paths.get(resultsDir, s"metadecomp-opt-" + args(0) + s"-$benchmark-$getTimestamp.csv")
			Files.write(
				resultsPath,
				...
				sqlFilesInBenchmark(benchmark).filter(f => if args.size >= 3 then f.getName.matches(args(2)) else true).foreach(sqlFile =>
					...
					if (Files.exists(Paths.get(benchmarkPath, "cardinalities", s"${queryName}.csv"))) {
						...
						if (metaGYO(sqlIR.hyperedges).isEmpty) { // Cyclic
							...
							while ( {
								meta = args(0) match {
									case "ip" => MetaDecompGraphConstructInterpolatable().run(hypergraph, width)
									case "baseline" => MetaDecompGraphConstructor().run(hypergraph, width)
									case "randomized" => MetaDecompGraphRandomizedConstructor().run(hypergraph, width)
									case _ => MetaDecompGraphConstructor().run(hypergraph, width)
								}
							...
							val metaGraphTime = (for (i <- 0 until repeatTimes) yield {
								...
								args(0) match {
									case "ip" => MetaDecompGraphConstructInterpolatable().run(hypergraph, width)
									case "baseline" => MetaDecompGraphConstructBaseline().run(hypergraph, width)
									case "randomized" => MetaDecompGraphRandomizedConstructor().run(hypergraph, width)
									case _ => MetaDecompGraphConstructor().run(hypergraph, width)
								}
