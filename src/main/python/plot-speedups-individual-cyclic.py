...
for benchmark in ['dsb-cyclic', 'musicbrainz-cyclic', 'subgraph-matching']:# benchmarks:
    for baseline in ['dpconv', 'duckdb']: #, 'uniondp', 'yanplus', 'learned-rewrite', 'llm-r2']:
        for metric in ['exec_time', 'total_time'] if baseline == 'dpconv' else ['total_time']:
            ...
            if metric == 'exec_time':
                if 'opt_exec_time' in base_df.columns:
                    base_timeout = (base_df['opt_exec_time'] == 0) & (base_df['exec_time'] == 0)
                else:
                    base_timeout = base_df['exec_time'] == 0
            ...
            if metric == 'exec_time':
                if 'opt_exec_time' in base_df.columns:
                    base_val = np.where(base_df['opt_exec_time'] != 0, base_df['opt_exec_time'], base_df['exec_time'])
                else:
                    base_val = base_df['exec_time']
                base = np.clip(base_val, None, 3e8)
            else:
                base = np.clip(base_df[metric], None, 3e8)
            ...
            if baseline == 'learned-rewrite' or baseline == 'llm-r2':
                ...
                small_step = 0.5
                small_max = 2.1
                large_powers = [10, 100, 1000, 10000, 100000]
            ...
            metric_prefix = 'exec' if metric == 'exec_time' else 'overall'
            plt.savefig(os.path.join(save_path, f'{metric_prefix}-speedups-over-{baseline}-{benchmark}.pdf'), format='pdf', transparent=True)
            # plt.show()
