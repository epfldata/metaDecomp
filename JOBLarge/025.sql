SELECT mc_1.note, t_1.title, t_1.production_year, cn_2.name, mi_idx_2.info, t_2.title
...
WHERE it_1.info = 'top 250 rank'
  AND mc_1.note NOT LIKE '%(as Metro-Goldwyn-Mayer Pictures)%'
  AND ( mc_1.note LIKE '%(co-production)%' OR mc_1.note LIKE '%(presents)%' )
  ...
  AND mi_idx_1.info_type_id = it_1.id
...
GROUP BY mc_1.note, t_1.title, t_1.production_year, cn_2.name, mi_idx_2.info, t_2.title;