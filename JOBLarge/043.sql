SELECT chn_1.name AS uncredited_voiced_character_1,
       t_1.title AS russian_movie_1,
       chn_2.name AS uncredited_voiced_character_2,
       t_2.title AS russian_movie_2
...
WHERE ci_1.note LIKE '%(voice)%'
  ...
  AND ci_2.note LIKE '%(voice)%'
  AND ci_2.note LIKE '%(uncredited)%'
